...
#### Persistent behavior in oc mirror v2
- The target (`file://`) directory will also hold the `working-dir` environment. This directory contains the various necessary data to build, update, and maintain cluster resources. It's best to keep this directory safe, and not modify it. It will be used again for incremental updates and additions to your cluster.
    
    !!! info "Scenarios"
        - No working-dir, no cache: it's like doing the mirror from scratch
        - Working-dir, no cache: incremental tar but all images downloaded again
        - No working-dir, cache: will download only new images but tar will be full mirror size
        - Working-dir, cache: incremental tar and will download only new images
...
- Be aware of the caching system, this will also take up considerable space on the disk depending on how many images you want to mirror.
    ...
    !!! question "Caching"
        ...
        - The cache takes up a lot of disk space can it be deleted?
            - Yes the cache can be removed, however it's best to keep it intact as this mechanism will allow for better incremental mirroring as images that exist in cache will not need to be downloaded. It is prunable using oc mirror deletes.
...
2. Perform a dry-run of the mirror to disk process to verify your imageset-config is valid and the tools can gather the data
...
3. Perform the mirror to disk process    
    ```{ .bash }
    ...
    ```{ .bash }
    oc mirror -c imageset-config.yaml file:///opt/4.17-mirrordata --cache-dir /opt/ocp-cache --v2
    ```
    - If you want to run multiple mirror's simultaneously using different imageset configs, specify it in the command by changing the imageset, port, and destination. The default port is `50000`
    ```{ .bash }
    oc mirror -c cluster2-imageset-config.yaml file:///opt/4.17-cluster2-mirrordata --cache-dir /opt/ocp-cache --port 50001 --v2
    ```
...
4. List your output directory and verify the image set `mirror_000001.tar` file was created. The `working-dir` will contain logs and relavent info for the data mirrored to disk.
    ```{ .bash }
...
5. The working-dir will also show you the exact version of OpenShift you mirrored, if you specified the platform mirror via `stable-4.xx` in your imageset config file.
...
6. You can also view exactly what Operator channel and Version was pulled during the mirror process. Useful to know and if using ODF and want to upgrade between `.z` stream releases, it's imperative to mirror current and upgrade versions to the mirror registry.
  ```{ .bash }
  find /opt/4.17-mirrordata/working-dir/operator-catalogs -path '*/filtered-catalogs/*/catalog-config/*/catalog.json' \
    -exec grep -l 'olm.package' {} \; | while read f; do
      jq -r 'select(.schema == "olm.package") | "\(.name)\t\(.defaultChannel)"' "$f"
      jq -r 'select(.schema == "olm.channel") | "\(.package)\t\(.name)\t\(.entries[].name)"' "$f"
  done
  ```
...
!!! note
    If you used the `rhel-oc-tools.sh` script and chose to download the installer you do not need to do this.
...
Place these files on a disk and transfer them to your disconnected network. You do not need to bring the `working-dir` over to the disconnected network. It will be recreated from the contents within the tarball.

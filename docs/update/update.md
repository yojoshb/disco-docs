...
1. Apply the cluster recources generated by the oc-mirror plugin. This will make your cluster aware of the catalog resources we mirrored earlier so you can utilize OperatorHub to install operators. This will overwrite the resources you applied early when installing the cluster.
```{ .bash }
...
## OperatorHub Refresh
By default OperatorHub will scan your mirror registry every 30 minutes or so. If you want the catalog server to immediately fetch the new bundles, you can force it's hand.

1. Find the catalog source pods on the cluster
```bash
oc get pods -n openshift-marketplace
```
```{ . .no-copy title="Example Output" }
NAME                                   READY   STATUS    RESTARTS   AGE
marketplace-operator-d8bfdb9df-vnzht   1/1     Running   0          3d9h
redhat-operators-qtd44                 1/1     Running   0          3d9h  # In this case the only catalog pod running is redhat-operators-xxxx 
```

2. Remove the catalog source pod
```bash
oc delete pod redhat-operators-qtd44 -n openshift-marketplace
```
```{ . .no-copy title="Example Output" }
pod "redhat-operators-qtd44" deleted
```

3. It will recreate and refresh the marketplace catalog
```bash
oc get pods -n openshift-marketplace
```
```{ . .no-copy title="Example Output" }
NAME                                   READY   STATUS              RESTARTS   AGE
marketplace-operator-d8bfdb9df-vnzht   1/1     Running             0          3d9h
redhat-operators-p68fs                 0/1     ContainerCreating   0          4s
```

If you have any operators with automatic install plans, this will kick off operator upgrades if there are new versions in the catalog